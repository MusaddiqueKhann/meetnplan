...
export default function Profile({ onNavigate, onLogout, bookings = [], deleteBooking, user }) {
  // change password
  ...
  async function handleDeleteAccount(e) {
    ...
    try {
      ...
      // onAuthStateChanged in App.jsx detects sign-out and shows SplashPage
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-6 pb-10">
      ...
      {/* User info card */}
      <div className="bg-white border border-neutral-200 rounded-2xl p-6 flex items-center gap-5">
        <div className="w-16 h-16 rounded-2xl bg-black flex items-center justify-center flex-shrink-0 overflow-hidden">
          {user.photoURL ? (
        ...
        <div className="flex flex-col gap-2 flex-1">
          ...
          {user.isGoogleUser && (
            <span className="text-[11px] text-neutral-400 bg-neutral-100 px-2 py-0.5 rounded-full w-fit">
      ...
      <div className="bg-white border border-neutral-200 rounded-2xl overflow-hidden">
        ...
        {myMeetings.length === 0 ? (
          ...
          <div className="divide-y divide-neutral-100">
            {myMeetings.map(b => (
              <div key={b.id} className="px-6 py-4 flex items-center gap-4 group hover:bg-neutral-50 transition-colors">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-neutral-400 truncate">{b.room}</p>
                </div>
                <button onClick={() => deleteBooking(b.id)}
                  className="w-7 h-7 flex items-center justify-center rounded-lg text-neutral-300 hover:bg-red-50 hover:text-red-400 transition-colors"
      ...
      {/* Google users — password managed by Google */}
      {user.isGoogleUser && (
          ...
          {!deleteConfirm ? (
            <div className="flex items-center justify-between gap-4">
              <p className="text-[13px] text-neutral-500">
                Permanently removes your account and all your meetings. This cannot be undone.
              </p>
              <button onClick={() => setDeleteConfirm(true)}
                className="flex-shrink-0 flex items-center gap-2 px-4 py-2.5 text-[13px] font-semibold text-red-500 border border-red-200 rounded-xl hover:bg-red-50 active:scale-[0.98] transition-all">
