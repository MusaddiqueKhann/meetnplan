import { LayoutDashboard, Calendar, Building2, PlusCircle, ChevronRight, Zap, CalendarClock, X, LogOut, ShieldCheck } from 'lucide-react'
...
export default function Sidebar({ currentPage, onNavigate, onOpenModal, rooms = [], bookings = [], checkedRooms = new Set(), toggleRoom, isOpen, onClose, onLogout, user }) {
  const now         = new Date()
  ...
  const isRoomLive = (roomName) => bookings.some(b =>
    b.room === roomName && b.date === todayStr &&
    nowMinutes >= b.startMinutes && nowMinutes < b.endMinutes
  )

  const nextBooking = bookings
    .filter(b => b.date > todayStr || (b.date === todayStr && b.endMinutes > nowMinutes))
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => onNavigate('profile')}
              ...
              <div className="flex-1 min-w-0 text-left">
                <p className="text-[10px] font-semibold text-[#BBBBBB] uppercase tracking-wider leading-none mb-0.5">
                  Welcome back
                </p>
